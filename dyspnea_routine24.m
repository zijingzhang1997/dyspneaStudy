...
function dyspnea_part(dataPath,CaseNum)        
...
toff=[10:round(length(bioAb)/fsbio)-5]';
...
if size(tNoise) ~= 0
    ind1=[1: tNoise(2)*fsDS];
    ind2=[tNoise(3)*fsDS: tNoise(4)*fsDS];
    ind=[ind1 ind2];
    Ncs(ind,:)=[];
    Bio(ind,:)=[];
end
