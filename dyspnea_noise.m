clear all
dataPath=['C:\Sleep test\dyspnea\data\10_12\'];
CaseNum=[10;5];
...
if size(tNoise) ~= 0
    ind=[tNoise(1)*fsDS: tNoise(2)*fsDS];
    Ncs(ind,:)=[];
    Bio(ind,:)=[];
end
tOff=((0:(length(Bio(:,1))-1))/fsDSbio)';

h(1)=figure;
sz=10;
subplot(6,1,1);
plot(tOff,Ncs(:,1),'LineWidth',0.5,'color','red');
...
subplot(6,1,2);
plot(tOff,Ncs(:,2),'LineWidth',0.5,'color','green');
xlabel('time (s)','FontSize',sz)
ylabel('Amp (a.u.)','FontSize',sz)
%xlim([0 168])
...
subplot(6,1,3);
plot(tOff,Ncs(:,3),'LineWidth',0.5,'color','red');
...
subplot(6,1,4);
plot(tOff,Ncs(:,4),'LineWidth',0.5,'color','green');
...
subplot(6,1,5);
plot(tOff,Bio(:,1),'LineWidth',0.5,'color','red');
xlabel('time (s)','FontSize',sz)
ylabel('Amp (a.u.)','FontSize',sz)
%xlim([0 168])
...
subplot(6,1,6);
plot(tOff,Bio(:,2),'LineWidth',0.5,'color','green');
xlabel('time (s)','FontSize',sz)
ylabel('Amp (a.u.)','FontSize',sz)
%xlim([0 168])
...
[cor1,h(3),ampfilt1New] = brEstCor(ampfilt1,fsDS,CaseNum);
[cor2,h(4),ampfilt2New] = brEstCor(ampfilt2,fsDS,CaseNum);

[brNcs,NcsPk,varNcs,varNcsFeature] = brEstAvg(ampfilt1New,fsDS,opts3);


[brBio,BioPk,varBio,varBioFeature] = brEstAvg(ampfilt2New,fsDS,opts3);
...
figName3 = [dataPath,'fig\',fileName,'corr1'];
print(h(3),[figName3,'.tiff'],'-dtiff','-r300');
savefig(h(3),[figName3,'.fig']);
figName4 = [dataPath,'fig\',fileName,'corr2'];
...
BR=struct('brBio',brBio,'BioPk',BioPk,'varBio',varBio,'varBioFeature',varBioFeature,'brNcs',brNcs,'NcsPk',NcsPk,...
    ...
    'BioSelect',BioData,'HRVfeature',HRV,'period',period,'tNoise',tNoise,'rNCSBio',rNCSBio);
